    ...
    public static void main(String[] args) {
        try {
            ...
            * 3 路由key topic模式下#号匹配一个或多个单词 *号匹配一个单词
             */
            channel.queueBind(QUEUE_NAME1,EXCHANGE_NAME,"direct_q1");
            channel.queueBind(QUEUE_NAME2,EXCHANGE_NAME,"direct_q2");
            //开启发布确认
            channel.confirmSelect();
            //3.发送消息
            String msg = "你好世界 路由模式";
            channel.basicPublish(EXCHANGE_NAME, "direct_q1", null, msg.getBytes());
            boolean result = channel.waitForConfirms();
            if (result) {
                System.out.println("direct模式消息投递成功");
            } else {

    ...
    public static void main(String[] args) {
        try {
            ...
            * 3 路由key topic模式下#号匹配一个或多个单词 *号匹配一个单词
             */
            channel.queueBind(QUEUE_NAME1,EXCHANGE_NAME,"*.baidu.*");
            channel.queueBind(QUEUE_NAME2,EXCHANGE_NAME,"jd.#");
            //开启发布确认
            channel.confirmSelect();
            //3.发送消息
            String msg = "你好世界";
            channel.basicPublish(EXCHANGE_NAME, "www.baidu.com", null, msg.getBytes());
            channel.basicPublish(EXCHANGE_NAME, "jd.com", null, msg.getBytes());
            boolean result = channel.waitForConfirms();
            if (result) {
                System.out.println("topic模式消息投递成功");
            } else {
